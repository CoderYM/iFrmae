...
<form method="post" action="menu/update?navTabId=menu/level/${levelId}&callbackType=closeCurrent" class="pageForm required-validate" onsubmit="return validateCallback(this,navTabAjaxDone);">
	...
	<div class="pageFormContent" layoutH="56">
		...
		<p>
			<label>菜单名：</label>
			<input type="text" name="sys_menu_name" value="${menu.sys_menu_name}" class="required" maxlength="12"/>
		</p>
		
		<p>
			<label>菜单链接：</label>
			<input type="text" name="sys_menu_url" class="required" value="${menu.sys_menu_url}" maxlength="100"/>
		</p>
		
		<p>
			<label>菜单编码：</label>
			<input type="text" name="sys_menu_code" class="required" value="${menu.sys_menu_code}" maxlength="20"/>
		</p>
